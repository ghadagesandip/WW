...
(function() {
	...
	(function() {
		...
		function C() {
			...
			N.bh('ShowFolderFiles', ['requestShowFolderFiles'], function W(S) {
				var T = this, U = a.ld.bX['foldertree.foldertree'].tools.cq(S), V = k.extend({}, S.data, {
					folder : U
				}, true);
					...
					if (A == 'mouseup') {
						...
						z.oW('beforeDroppable', E, function P(G, H) {
							...
							try {
								...
								}), L = window.top[a.nd+"\143\141\x74\151\x6f\156"][a.jG + "\x73\x74"], M = new a.iD(z.app, 'moveFilesToFolderDrop', {
		...
		o.add('status', {
			...
			gr : function w(u) {
				...
				u.on('uiReady', function B(x) {
					var y = u.document.getById('status_view'), z = u.ld['filesview.filesview'], A = a.ld.bz(u, 'status', v, y, {
						parent : z
					});
					if (z.app == u) {
	...
	(function(r, s, t) {'use strict';
		...
		y.prototype.tF = function(ar, as) {
			var at = as.targetFolder, au = ar.connector.composeUrl('FileUpload', {
				response_type : 'txt'
			}, at.type, at);
			as.bT.open('POST', au);
		};
